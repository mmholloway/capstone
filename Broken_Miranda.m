...
[denoise_test,igram1_denoised_coeffs,igram1_org_coeffs,s,shifts] = wdenoise2(img(:,:,1),'NoiseEstimate','LevelDependent','Wavelet','sym6');

[denoise_test1,igram1_denoised_coeffs1,igram1_org_coeffs1,s1,shifts1] = wdenoise2(img(:,:,1));

figure;
axis on;
imshowpair(denoise_test.',denoise_test1.');
title("Interferogram #1 After wdenoise2()")
set(gca,'FontSize',16)

denoise_land = denoise_test.*double(boundaries_logical);
denoise_land1 = denoise_test1.*double(boundaries_logical);
...
single_noisy_land1 = zeros(size(denoise_land1,1),size(denoise_land1,2));
single_noisy_land((denoise_land < mean(denoise_land,"all")-(std(denoise_land,1,"all")/2)) & (boundaries_logical == 1)) = 1;
single_noisy_land1((denoise_land1 < mean(denoise_land1,"all")-(std(denoise_land1,1,"all")/2)) & (boundaries_logical == 1)) = 1;

figure;
imshowpair((single_noisy_land.*255).',(single_noisy_land1.*255).');
title("Isolated Noisy Land Pixels")
set(gca,'FontSize',16)
...
nexttile
imshowpair(denoise_land.',denoise_test1.');
title("Interferogram #1 Land Only After wdenoise2()")
set(gca,'FontSize',16)
